...
const CouldNotCreateUserMsg = "sorry, an error occured and your account could not be created"
...
func (s *UserService) CreateUser(ctx context.Context, u *NewUser) error {
	if u.Password == "" {
	...
	userEntry := storage.User{Nick: u.Nick, PwHash: string(pwHash)}
